...
public class ComposicaoDAO extends ConexaoDB {
	...
	public Composicao insert(Composicao entidade) {
		try (PreparedStatement preparedStatement = prepararSQL(INSERT_COMPOSICAO_SQL,
			...
			preparedStatement.setLong(1, entidade.getExame_id().getId());
			preparedStatement.setLong(2, entidade.getComposicao_exame_id().getId());
			preparedStatement.setLong(3, entidade.getValor_referencia_composicao_exame_id().getId());
	...
	public Composicao findById(long id) {
		...
		try (PreparedStatement preparedStatement = prepararSQL(SELECT_COMPOSICAO_BY_ID)) {
			...
			while (rs.next()) {
				...
				entidade = new Composicao(
						id,
						servicoExame.buscarPorId(exameId),
						servicoComposicaoExame.buscarPorId(composicaoExameId),
						servicoValorReferenciaComposicaoExame.buscarPorId(valorReferenciaComposicaoExameId));
			}
		} catch (SQLException e) {
			printSQLException(e);
		} catch (ClassNotFoundException e) {
	...
	public void updateComposicao(Composicao entidade) throws SQLException {
		try (PreparedStatement statement = prepararSQL(UPDATE_COMPOSICAO_SQL)) {
			statement.setLong(1, entidade.getExame_id().getId());
			statement.setLong(1, entidade.getComposicao_exame_id().getId());
			statement.setLong(2, entidade.getValor_referencia_composicao_exame_id().getId());
